...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CFG_MTU_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "wait now until ESP_GAP_BLE_AUTH_CMPL_EVT");
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16){
            ...
            ESP_LOGI(GATTC_TAG, "UUID32: %x", p_data->search_res.srvc_id.uuid.uuid.uuid32);
        } else if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_128){
            esp_gatt_id_t *srvc_id = &p_data->search_res.srvc_id;
            ESP_LOGI(GATTC_TAG, "UUID128: %x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x",       srvc_id->uuid.uuid.uuid128[0],
        ...
        // if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 && p_data->search_res.srvc_id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) {
            ESP_LOGI(GATTC_TAG, "service found, start_handle 0x%X end_handle 0x%X", p_data->search_res.start_handle, p_data->search_res.end_handle);
            get_server = true;
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "get_server %d", get_server);
        if (get_server){
            uint16_t count = 0;
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
                        ...
                        ESP_LOGI(GATTC_TAG, "esp_ble_gattc_register_for_notify 0x%X, found with notify", char_elem_result[0].char_handle);
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_INDICATE)){
                        ...
                        ESP_LOGI(GATTC_TAG, "esp_ble_gattc_register_for_notify 0x%X, found with indicate", char_elem_result[0].char_handle);
                        esp_ble_gattc_register_for_notify (gattc_if, gl_profile_tab[PROFILE_A_APP_ID].remote_bda, char_elem_result[0].char_handle);
                    }
                }
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            ESP_LOGI(GATTC_TAG, "ESP_GATTC_REG_FOR_NOTIFY_EVT count = %d", count);
            if (count > 0){
                ...
                if (!descr_elem_result){
                    ...
                    ESP_LOGI(GATTC_TAG, "esp_ble_gattc_write_char_descr (notify) 0x%X", descr_elem_result[0].handle);
    ...
    case ESP_GATTC_NOTIFY_EVT: {
        ...
        if (p_data->notify.is_notify){
            if(!buffer_download_count){
                ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, first");
            ...
            // esp_log_buffer_hex(GATTC_TAG, &buffer_download[buffer_download_count], p_data->notify.value_len);
            buffer_download_count++;
        }else{
            ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, done");
            ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, receive indicate value:");

            ESP_LOGI(GATTC_TAG, "buffer_download_count %d, time difference %d", buffer_download_count, (uint16_t) (esp_timer_get_time() - time_measure));
        }
        // esp_log_buffer_hex(GATTC_TAG, p_data->notify.value, p_data->notify.value_len);

        if (!p_data->notify.is_notify){
            ESP_LOGI(GATTC_TAG, "now disconnecting");
            esp_gatt_status_t ret_status = esp_ble_gattc_close(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, 0);
            if (ret_status != ESP_GATT_OK){
                ESP_LOGE(GATTC_TAG, "esp_ble_gattc_close, error status %d", ret_status);
            }
        }
        break;
    }
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "write descr success handle 0x%X", p_data->write.handle);
        ...
        ESP_LOGI(GATTC_TAG, "esp_ble_gattc_write_char 0x%X", gl_profile_tab[PROFILE_A_APP_ID].char_handle_2A52);
